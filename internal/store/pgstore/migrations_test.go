...
// already holding a team's roster, sitting on 001, is brought to 002.
...
// Rolling 002 back must not take the rest of the schema with it. A
// migration whose Down is wrong is only discovered when somebody needs it.
func TestMigrationsRollBackCleanly(t *testing.T) {
	...
	if hasColumn(t, s, "sprints", "capacity_reviewed_at") {
		t.Error("the rollback left its column behind")
	}
	...
	if !hasColumn(t, s, "sprints", "capacity_reviewed_at") {
		t.Error("re-applying did not restore the column")
	}
}
